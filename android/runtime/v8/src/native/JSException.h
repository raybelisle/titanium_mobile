...
{
public:
	static v8::Handle<v8::Value> Error(v8::Isolate* isolate, const char* msg)
	{
		return THROW(isolate, msg);
	}

	// Calling a constructor as a function not allowed.
	static v8::Handle<v8::Value> CalledConstructor(v8::Isolate* isolate)
	{
		return THROW(isolate, "Calling this constructor is not allowed.");
	}

	// An attempt to get the current JNI environment failed.
	static v8::Handle<v8::Value> GetJNIEnvironmentError(v8::Isolate* isolate)
	{
		return THROW(isolate, JNIENV_GET_ERROR_MSG);
	}

	static v8::Handle<v8::Value> fromJavaException(v8::Isolate* isolate, jthrowable javaException = NULL);
};
